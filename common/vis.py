...
class Visualizer:
    ...
    @classmethod
    def initialize(cls, visualize, vis_path):
        cls.visualize = visualize
        if not visualize:
            return

        # cls.colors = {'red': (174, 35, 52), 'blue': (79, 91, 189)}

        # cls.colors = {'red': (226, 49, 67), 'blue': (32, 74, 151), 'green': (91, 198, 199)}
        cls.colors = {'red': (226, 49, 67), 'blue': (56, 108, 205), 'green': (91, 198, 199)}
    ...
    def visualize_prediction_batch(
        ...
        qry_mask_b = utils.to_cpu(qry_mask_b)  # GT mask
        pred_mask_b = utils.to_cpu(pred_mask_b)  # predicted mask
        cls_id_b = utils.to_cpu(cls_id_b)
        cls_id_b += 1

        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, query_name) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, query_name)):
            iou = iou_b[sample_idx] if iou_b is not None else None
            cls.visualize_prediction(
                spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, True, iou, query_name)
    ...
    def visualize_prediction(
        ...
        spt_color = cls.colors['blue'] # support cam
        qry_color = cls.colors['green'] # query cam
        pred_color = cls.colors['red'] # seg
        ...
        # for coco vis
        if 'COCO' in query_name:
            slash_index = query_name.find("/")
            dot_index = query_name.rfind(".")
            query_name = query_name[slash_index + 1:dot_index]

        # merged_pil.save(cls.vis_path + '%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '.jpg')
        merged_pil.save(cls.vis_path + 'class-%d_iou-%.2f-%s' % (cls_id, iou, query_name) + '.jpg')
    ...
    @classmethod
    def apply_mask(cls, image, mask, color, alpha=0.55):
